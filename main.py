...
save_path = r'C:\Users\HW\PycharmProjects\threading\ecmo_ai_model_221007.pickle'
...
def savitzky_golay(y, window_size, order, deriv=0, rate=1):
    try:
        ...
        raise ValueError("window_size and order have to be of type int") from expt
                        ...
                        for j in range(i + 2, len(R), 2):
                            if R[j:j + 2] == '8f':
                                ...
                                if self.serial_loop_n % 4 == 0:
                                    ...
                                    if len(self.ibp_wave_arr) > 1000:
                                        self.ibp_wave_arr = np.array(self.ibp_wave_arr[1::])
                                    # print("ibp: ", *self.ibp_wave_arr)

                                    self.ibp_diff_arr = np.append(self.ibp_diff_arr, ibp_diff)
...
class ApplyDNN(threading.Thread):
    ...
    def run(self):
        ...
        while 1:
            ...
            # print('index: ', index)
            # print(serial_loop_n, self.dnn_loop_n)
            if 1000 >= len(ibp_wave_arr) >= 125 and 1000 >= len(pump1_arr) >= 90 and 1000 >= len(pump2_arr) >= 90:
